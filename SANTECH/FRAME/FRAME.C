...
{ static unsigned char score;
  ...
  { if(sci_input_buffer == _home_bonus)       score = score & 1 ? score & ~1 : score | 1;
	...
	else if(sci_input_buffer == _possession)  score = score >= 2 ? 1 : 2;
  }
}
...
{ static unsigned char score, score_10, score_1, buffer;
  ...
  { if(!mode && sci_input_buffer == _shot)
    { score = (score + 1) % 200;
      score_10 = score/10 ? font[score / 10] : 0;
	  score_1 = font[score % 20];
	}
	else if(!mode && sci_input_buffer == _set) mode = _set;
	else if(mode == _set && sci_input_buffer == _shot)
	{ mode = _shot;
	  blink_character = 13;
	  blink_segments = 0;
	  buffer = 0;
...
{ static unsigned char score, score_10, score_1, buffer;
  ...
  { if(!mode && sci_input_buffer == _empty)
    { score =(score + 1) % 200;
      score_10 = score/10 ? font[score / 10] : 0;
	  score_1 = font[score % 20];
	}
	else if(!mode && sci_input_buffer == _set) mode = _set;
	else if(mode == _set && sci_input_buffer == _empty)
	{ mode = _empty;
	  blink_character = 10;
	  blink_segments = 0;
	  buffer = 0;
