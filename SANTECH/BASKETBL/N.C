...
#define _MIDDLE_DIGIT_NUMBER_  10  // bottom board start digit number.
...
void bonus_possession(unsigned char flag)
{ static unsigned char score;
  ...
  { if(sci_input_buffer == _home_bonus)       score = score & 1 ? score & ~1 : score | 1;
    else if(sci_input_buffer == _guest_bonus) score = score & 2 ? score & ~2 : score | 2;
	else if(sci_input_buffer == _possession)  score = (score>>2)>1 ? score & 3 : score + 4;
  }
}
...
void home_fouls(unsigned char flag)
{ static unsigned char score, score_1, buffer;
  ...
  { if(!mode && sci_input_buffer == _home_fouls)
    { score = (score + 1) % 20;
      score_1 = font[score];
	}
	else if(!mode && sci_input_buffer == _set) mode = _set;
	else if(mode == _set && sci_input_buffer == _home_fouls)
	{ mode = _home_fouls;
	  blink_character = 13;
	  blink_segments = 0;
	  buffer = 0;
...
void guest_fouls(unsigned char flag)
{ static unsigned char score, score_1, buffer;
  ...
  { if(!mode && sci_input_buffer == _guest_fouls)
    { score = (score + 1) % 20;
      score_1 = font[score];
	}
	else if(!mode && sci_input_buffer == _set) mode = _set;
	else if(mode == _set && sci_input_buffer == _guest_fouls)
	{ mode = _guest_fouls;
	  blink_character = 14;
	  blink_segments = 0;
	  buffer = 0;
...
void player_foul(unsigned char flag)
{ static unsigned char buffer[3], player[5][3], counter;
  ...
  if(flag == _INITIALIZATION_)
  { counter = 0;
    for(i=0;i<5;i++) for(j=0;j<3;j++) player[i][j] = 0;
	for(j=0;j<3;j++) buffer[j] = 0;
	for(i=0;i<3;i++) data[i+10] = &player[counter][i];
  }
  else if(flag == _NEXT_CHARACTER_)
  { if(!mode && sci_input_buffer == _player)
    { counter = counter < 4 ? counter + 1 : 0;
	  for(i=0;i<3;i++) data[i+10] = &player[counter][i];
	}
	else if(!mode && sci_input_buffer == _set) mode = _set;
	else if(mode == _set && sci_input_buffer == _player)
	{ mode = _player;
	  blink_character = 12;
	  blink_segments = 0;
	  for(j=0;j<3;j++) buffer[j] = 0;
	  for(i=0;i<3;i++) data[i+10] = &buffer[i];
	}
	...
	{ if(sci_input_buffer == _clear)
	  ...
	  { mode = 0;
	    blink_character = 0;
		for(i=0;i<3;i++)
		{ player[counter][i] = buffer[i];
		  data[i+10] = &player[counter][i];
		}
	  }
	  else if(sci_input_buffer >= _1 && sci_input_buffer <= _0)
	  { for(i=0;i<2;i++) buffer[i] = buffer[i+1];
	    buffer[2] = font[sci_input_buffer % 10];
		if(buffer[0] == font[0]) buffer[0] = 0;
	  }
	}
  }
}
